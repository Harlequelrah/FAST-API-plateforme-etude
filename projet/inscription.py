...
async def   read_inscription(id_Etud: int, id_Cours: int, db: Session = Depends(get_db)):
...
async def   delete_inscription(id_Etud: int, id_Cours: int, db: Session = Depends(get_db)):
