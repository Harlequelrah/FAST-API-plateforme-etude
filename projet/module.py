...
def read_module(id_Module: int, db: Session = Depends(get_db)):
    return crud.get_module(db,id_Module)

@app_module.get("/modules/getall",response_model=list[schemas.Module])
def read_module( skip:int=0,limit:int=100,db:Session=Depends(get_db)):
...
def get_module_of_cours(id_Cours: int, db: Session = Depends(get_db)):
